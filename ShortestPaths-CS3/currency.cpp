...
int main(int argc, char** argv)
{
    ...
    {
        weights[i] = -log(weights[i] * (1 - transCost));
    }
    ...
    if(detector == -1)
    {
        ...
        for(int i = 0; i<size-1; i++)
        {
            ...
            fout << firstVert<<" " << secVert<<" " << exp(-weights[weightIdx])<<" " << eLabels[weightIdx]<<" " << endl;
            edgeTotal = dist[dest];
            
        }
        cout<<"Runtime: " <<durNS.count() << " microseconds"<<endl;
        cout<<"Effective Exchange Rate " << exp(-edgeTotal) << endl;
        delete[] path;
    }
    else
    {
        int* cycle;
        int cycleSize = getCycle(detector, prev, vert, cycle);
